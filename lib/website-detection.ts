...
export function detectWebsite(
  ...
  if (websiteIdFromBody) {
    ...
    if (config) {
      console.log(
        `Website detected from request body websiteId: ${config.name}`
      );
      return config;
    }
  }
  ...
  if (origin) {
    ...
    if (domain) {
      ...
      if (config) {
        console.log(
          `Website detected from Origin header: ${config.name} (${domain})`
        );
        return config;
      }
    }
  ...
  if (referer) {
    ...
    if (domain) {
      ...
      if (config) {
        console.log(
          `Website detected from Referer header: ${config.name} (${domain})`
        );
        return config;
      }
    }
  ...
  if (host) {
    ...
    if (config) {
      console.log(
        `Website detected from Host header: ${config.name} (${domain})`
      );
      return config;
    }
  }
  ...
  if (iframeSrc) {
    ...
    if (domain) {
      ...
      if (config) {
        console.log(
          `Website detected from X-Iframe-Src header: ${config.name} (${domain})`
        );
        return config;
      }
    }
...
export async function createWebsiteContext(
  ...
  const { loadWebsiteData, loadWebsiteSystemPrompt } = await import(
    "./website-config"
  );
 ...
 */
export function logDetectionDetails(
  request: Request,
  detectedConfig: WebsiteConfig,
  websiteIdFromBody?: string
): void {
  const headers = {
