 ...
 */
export function detectWebsiteFromReferrer(referrer: string): WebsiteConfig | null {
 ...
 */
export async function loadWebsiteData(websiteConfig: WebsiteConfig): Promise<string> {
  try {
 ...
 */
export async function loadWebsiteSystemPrompt(websiteConfig: WebsiteConfig): Promise<string> {
  try {
    const promptPath = path.join(process.cwd(), 'prompts', websiteConfig.systemPromptFile)
    const content = await fs.readFile(promptPath, 'utf-8')
    return content
  } catch (error) {
    console.error(`Failed to load system prompt for ${websiteConfig.id}:`, error)
