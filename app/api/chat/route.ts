...
import { detectWebsite, createWebsiteContext, logDetectionDetails } from '@/lib/website-detection'
...
export async function POST(req: Request) {
  try {
    const { messages, websiteId, lang }: { messages: CoreMessage[]; websiteId?: string; lang?: string } = await req.json()
    ...
    console.log(`Making request to OpenRouter for ${websiteInfo.name} with ${messages.length} messages`)
